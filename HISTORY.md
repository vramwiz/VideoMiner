...
## 2026-07-04 起動直後の右クリックサムネイル切替反応改善
- 引数なし起動で前回ファイルが NAS 上にあり開かない経路を通った直後、右クリックでサムネイル一覧へ切り替わらないことがある問題を調査した。
- Debug ログでは NAS 上の前回ファイル判定は数 ms 以内に終わっており、サムネイル切替処理自体も `Tab` キー経由ではすぐに入ることを確認した。
- 右クリック位置がシークバーの広めの当たり判定に入ると、チャプター切替位置ではないのに動画面右クリックとしても開始されず、サムネイル切替まで届かない場合があった。
- 右クリックのサムネイル切替開始判定を通常クリック判定から分け、シークバーの広めの余白では右クリックを動画面操作として受けられるようにした。
- シークバー上の有効な位置での右クリックチャプター切替は従来通り先に処理し、overlay ボタンや前後動画ボタン上ではサムネイル切替を開始しない。

## 2026-07-03 H キーで表示左右反転
- 表示だけを左右反転する操作を追加し、`H` キーで ON/OFF できるようにした。
- 再生中に切り替えた場合は現在位置を維持して表示を描き直し、必要ならその位置から再生を再開する。
- D3D 表示では shader の texture UV を左右反転し、GDI fallback では `StretchBlt` で描画だけを左右反転するようにした。
- README の基本操作と F1 ヘルプへ `H` キーの説明を追加した。

## 2026-06-29 メイン画面の右ボタン+ホイールフレーム移動
- メイン画面上で右ボタンを押しながらマウスホイールを回すと、1 フレーム目安で前後へ移動する操作を追加した。
- 通常のメイン画面ホイールは従来通り拡大/縮小のままとし、シークバー上ホイールの向きと同じ向きでフレーム移動するようにした。
- 右クリック単体のサムネイル一覧表示/非表示は維持し、右ボタン押下中にホイール操作した場合は MouseUp でサムネイル一覧を切り替えないようにした。
- 右ボタン+ホイールのフレーム移動時は、再生中なら一時停止して細かく確認できる状態にする。
- 右ボタン+ホイール操作時はシークバーを表示し、先頭/末尾でそれ以上移動できない場合は seek/再描画へ進まないようにした。
- README の基本操作へ右ボタン+ホイール操作と補足説明を追加した。
...
- セーフエリア表示切り替え時は、GDI `Invalidate` の前に D3D overlay 更新と現在フレームの D3D 再表示を試すようにした。
- セーフエリア表示中でも明るさ/コントラストの左上ステータスを D3D overlay 側へ載せ、ステータスだけ GDI fallback へ戻るちらつきを避けるようにした。

## 2026-06-28 チャプター区間ループの音声待ち終端修正
- チャプター A/B がある状態のループ再生で、0..A、A..B、B..終端の区間ごとに戻るべき先がずれることがある問題を調査した。
- 映像が音声より先行して区間終端付近で待機する分岐が、ループ区間終端でもファイル終端処理へ進み、現在区間の先頭ではなくファイル終端用のループ開始位置へ戻る可能性があった。
- 音声待ち終端でも、ループ中かつ有効なループ区間がある場合は `LoopSegmentStartMs` へ直接戻すようにして、通常の区間到達ループ判定と戻り先を揃えた。

## 2026-06-28 色調整ステータス表示のちらつき抑制
- 明るさ/コントラスト操作時に、動画本体の D3D 色補正反映と左上ステータス表示が別々に D3D 再表示を発生させ、現在値表示のちらつきや動画が一瞬白/黒に見えることがあった。
- 色補正値と一時ステータスをまとめて更新し、1 回の D3D frame 再表示で反映する経路を追加した。
- キー操作からの色調整は新しい一括更新経路を使うようにし、通常の汎用ステータス表示や初期色補正反映は従来の個別経路を維持した。
- 色調整直後は直近の D3D backbuffer を優先し、Paint が短時間だけ GDI fallback へ落ちないようにして、D3D/GDI の交互描画による全体ちらつきを抑えた。
- 再生中は色調整キー入力から現在フレームの即時 D3D 再表示を行わず、補正値とステータスだけ更新して次の通常 D3D 再生フレームに乗せるようにした。
- 通常の NV12 D3D 再生フレームにも色補正 constant buffer を設定し、補正が BGRX/ソフト変換側のフレームでだけ見える状態を解消した。
- `AV_PIX_FMT_YUV420P` の動画は NV12 専用判定で D3D 直表示から外れ、毎フレーム BGRX32 変換へ落ちていたため、YUV420P の U/V plane を D3D 用 2ch texture に詰めて既存 shader へ渡す経路を追加した。
- デバッグ再生ログで、対象の `fmt=0` 動画が `d3d_presented=True` となり、通常 D3D frame 表示へ乗ることを確認した。
- 確認:
  - Win64 Debug: 成功、警告 0 / エラー 0。
...
- 履歴タイル描画時に代表一覧が未ロードなら、フォルダ単位の worker で `FirstMediaFileInFolder` / `BuildForFile` を実行し、UI thread で `FFolderHistoryMediaLists` へ反映して再描画するようにした。
- 同じフォルダの worker が重複しないよう `FFolderHistoryMediaListLoading` を追加し、履歴更新時は generation と loading 状態をクリアして古い結果を捨てるようにした。
- 確認:
  - Win64 Debug: 成功、警告 0 / エラー 0。
  - `tools\EnsureUtf8Bom.ps1 -Check`: 成功。
...
- 時刻表示も同じ中心 Y へ揃え、GDI/D3D overlay state の基準になる矩形を統一した。
- 確認:
  - Win64 Debug: 成功、警告 0 / エラー 0。
