...
// 動画ファイル内の音声ストリームを FFmpeg で開くための補助ユニット。
// 音声デコーダ、音声フレーム、PCM 変換用 swr コンテキストの準備を担当する。
...
// 入力音声を PCM16 stereo 48kHz へ変換する swr コンテキストを準備する。
...
// 音声ストリームが存在する場合に音声デコーダと PCM 変換コンテキストを開く。
...
var
  ...
  OutLayout : TAVChannelLayout; // 出力 PCM のチャンネルレイアウト
