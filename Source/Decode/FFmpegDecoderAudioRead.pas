...
// FFmpeg から音声フレームを読み、PCM16 stereo 48kHz として取り出す。
// 音声シーク後のサンプル破棄や、実フレーム形式に合わせた resampler 再作成もここで扱う。
...
// 目標サンプル数に届くまで音声を読み、PCM バッファへ追加する。
...
// 音声ストリームを指定時刻へ戻し、シーク直後の不要サンプル破棄位置を設定する。
