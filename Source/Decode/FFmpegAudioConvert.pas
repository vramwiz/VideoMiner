...
// FFmpeg の音声フレームを PCM 出力用の固定形式へ変換する。
// swr_convert の呼び出しと出力バッファサイズ調整をこのユニットへ集約する。
...
// AVFrame の音声サンプルを PCM16 stereo 48kHz へ変換する。
...
function ConvertAudioFrameToPcm16Stereo48k(AudioFrame: PAVFrame; SwrContext: PSwrContext;
  SourceSampleRate: Integer; out Pcm: TBytes; out SampleCount: Integer): Boolean;
var
  OutData    : array[0..0] of PByte; // swr_convert へ渡す出力バッファポインタ
