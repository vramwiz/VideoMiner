...
2. ループ再生の先頭戻りを滑らかにする。
   - 当初より改善済みだが、ループ末尾から先頭へ戻る時にわずかな待ちが残る。
...
- 倍速再生時の音声同期を改善する。
  ...
  - `マイドキュメント\VideoMiner\VideoMiner_playback_debug.log` の `audio_pump`、`audio_tempo_fallback`、`playback_tick` を見て、音声変換、キュー補充、映像同期のどこで遅れているか切り分ける。
- チェック機能の UI を育てる。
  - チャプター種別を表示する。
