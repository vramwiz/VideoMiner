...
0. シークバー小 fallback はかなり減ったため、次は中央 overlay の操作感を整える。
   - `videominer_4k30_motion_debug.mp4`、`test_out_r270.mp4`、`videominer_4k30_debug.mp4` で出ていた小表示は、左右ファイル移動ボタン、回転後 bitmap、ロード中初期表示の対策で大きく減った。
   - 残る課題は、非常時 fallback を完全になくすよりも、D3D overlay と入力判定の違和感を減らすことを優先する。
   - 中央の先頭/10秒戻し/再生一時停止/10秒進み/末尾ボタンは、各ボタンの隙間で hover が切れるため、5つをまとめた長方形の操作領域として扱う。
...
4. 4K 以上のデコード表示経路を段階的に改善する。
   ...
   - D3D overlay 化の現状。
     ...
     - 2026-06-25: 旧 GDI / fallback seek bar の描画入口を塞いだ結果、余計な旧表示は消えたが、起動直後の停止状態ではマウスを動かしても新 D3D seek bar が表示されない。
      ...
      - 2026-06-25: 旧 GDI で画面中央に出していた先頭/10秒戻し/再生一時停止/10秒進み/末尾ボタンも D3D overlay state へ渡し、D3D backbuffer 上へ描くようにした。中央 overlay 表示中も `center_overlay_visible` で D3D 表示を止めない。
      - 2026-06-25: D3D present ログへ `transport=True/False` を追加した。中央操作ボタンが D3D 側で描かれているかの確認に使う。
      - 2026-06-25: 新 D3D 中央ボタンの見た目を旧 GDI 参考画像へ寄せ、背景を半透明四角、10秒前後を曲線矢印へ変更した。移行中は旧 GDI 中央ボタンの Paint も止め、旧表示が混じらないようにした。
