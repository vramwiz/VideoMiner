...
- 下側ツールグループに音量関連の操作を追加する。
  - 音量表示。
  - 音量変更。
  - ミュート切り替え。
- ループ再生の有無を切り替えられるようにする。
  - まずは現在ファイル単体のループを想定する。
  - 将来的にフォルダ内連続再生や一覧ループとどう分けるか検討する。
- 画面の中央水平位置の左右端に、前の動画 / 次の動画へ移動するオーバーレイボタンを追加する。
  - 既存のフォルダ内ナビゲーション `NavigateBy(-1 / 1)` を呼ぶ。
  - 中央の再生/スキップ系アイコンとは別の左右端ナビゲーションとして扱う。
  - 端のヒット領域に hover したときだけ表示する方針。
- ズーム表示とパン操作。
- 画像ファイル対応。
