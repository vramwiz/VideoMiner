...
1. 別環境のインストール版で `Runtime error 217 at 00007ff614c32422` が出る原因を特定する。
   - 直前まで起動できていた環境で起動できなくなったため、新規 DLL 不足ではなく、配布物、インストール先、起動時初期化、DLL 混在を優先して疑う。
   - Portable ZIP 展開版は起動できる前提なので、開発環境にも配布 ZIP を展開した環境と同じ DLL 状態を作り、その状態で実行して再現を狙う。
