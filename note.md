...
- 2026-06-22 時点では、サムネイル一覧はバックグラウンド worker 方式を使わない。
  ...
  - バックグラウンド worker 方式は一度試したが安定しなかったため、今後の再挑戦課題として残す。
  - 再挑戦する場合も、現状の同期生成を壊さず、別ブランチ的に worker の寿命管理、FFmpeg デコーダのスレッド分離、UI 反映タイミングを切り分けて確認する。
...
- 2026-06-17 の作業終了時点では、サムネイル一覧モードは最小入口まで実装済み。
  ...
  - 2026-06-17 の次段階で、フォーム端のリサイズ操作を改善した。
    ...
    - 2026-06-24 に通常動画画面の右クリックは手動チャプタートグルへ変更した。サムネイル一覧は `Tab` で開く。
    - シークバー上の右クリック時、カーソル位置に手動チャプターがあれば削除し、なければその位置へ追加する。
...
- 2026-06-23 時点では、起動時のファイル読み込みをフォーム表示後へ遅延する 2 段階起動へ変更済み。
  - `Application.Run` 前に前回ファイルや引数ファイルを同期 open しない。
  - `VideoMiner.dpr` は `QueueStartupOpenFile` / `QueueStartupOpenRemembered` で起動後 open を予約する。
  - `VideoMinerMainForm` は `WM_VM_STARTUP_OPEN` を受けて、フォーム表示後に前回ファイルや引数ファイルを読み込む。
  - 読み込み中は `Loading last video...` / `Loading video...` を表示する。
  - 確認では、まず `VideoMiner - No video loaded` が表示され、その後 loading 表示を経て動画タイトルへ切り替わるため、実際の読み込み時間より起動が速く感じられる。
...
1. サムネイル生成のバックグラウンド worker 化を再挑戦する。
   - 次の優先課題として扱う。
   - UI タイマーによる同期生成では、大量の動画や重い動画で限界があるため、専用スレッド化を検討する。
   - ただし通常再生やシークのデコードを阻害しないことを最優先にする。
   - 再生中、シーク中、hover 実動画プレビューなど、メインのデコード処理が動いている間はサムネイル生成を一時停止する。
   - 現在の同期生成を壊さず、worker の寿命管理、専用デコーダ、キャンセル、UI 反映タイミングを小さく検証する。
2. ループ再生の先頭戻りを滑らかにする。
...
3. チェック関係と倍速再生時の音ズレ確認は、上記より後回しにする。
...
- 長い読み込み中に止まっていないことを見せる。
  ...
  - 実際の読み込み時間短縮とは別に、体感上の安心感を上げる UI 課題として扱う。
- 動画編集者用チェック機能を育てる。
...
- ループ先頭フレームキャッシュを再検討する。
  - 目的は、ループで先頭へ戻る時の seek/reopen 待ちをユーザーに見せず、先頭数フレームを滑らかに表示すること。
  - キャッシュ対象はファイルではなく、表示面へ渡しやすいデコード済み画像データとする。
  - 理想は、その動画を再生している間だけ有効な小さな一時キャッシュとすること。
  - 1 回目のループでは無理に事前生成せず、通常通りループ先頭またはチャプター先頭へ戻る。
  - ループで戻った直後にフラグを立て、その後に通常再生で表示された先頭側の 3-4 フレームを正しい順番で保存する。
  - 2 回目以降に同じループ先頭へ戻る時は、seek/restart の準備中に保存済みフレームを順番に表示し、止まって見える時間を埋める。
  - 既存の再生バッファを無理にリセットしたり差し替えたりするのではなく、通常デコーダが再開するまでの空白を、前回学習したループ先フレーム列で補う発想とする。
  - 通常デコーダが再開したらキャッシュ表示をやめ、通常フレーム表示へ自然につなぐ。
  - キャッシュキーは少なくとも動画ファイル、ループ開始位置、表示回転、ループ区間を含める。
  - 動画切り替え、チャプター変更、ループ区間変更、表示回転変更では破棄する。
  - 最初の実装は、キャッシュ済み先頭フレーム列のうち 1 枚目だけを即表示する最小版から始めてもよい。
  - 再挑戦時は、音声再開との同期、ループ区間が変わった時の破棄、動画切り替え時の破棄、alpha 合成済み/未合成の扱い、ズーム/パン状態との関係を確認する。
  - 先頭フレームキャッシュを表示している間に通常デコーダの seek を裏で進められるか、またはキャッシュ表示後にデコーダ位置を自然に接続できるかを重点的に見る。
...
- サムネイル生成のバックグラウンド worker 化。
  - 以前試した際は安定しなかったため、現状は `ThumbnailTimer` から 1 枚ずつ同期生成する方式に戻している。
  - 再挑戦時は、UI スレッドへ直接触らない、worker ごとに専用デコーダを持つ、キャンセル/破棄時に古い結果を捨てる、一覧を閉じた時に worker を確実に止める、という点を重点確認する。
  - worker 化する場合も、通常再生やシークのデコードを阻害しないよう、再生中、シーク中、hover 実動画プレビュー中は生成を一時停止する。
  - worker は生成要求を溜めるだけにし、UI 反映は main thread 側で生存確認と世代確認を行ってから反映する。
...
- 日付ごとの実装履歴と調査記録は `HISTORY.md` に分離する。
