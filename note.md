...
- `Source\Lib\ShortcutAction` をプロジェクトに追加し、キーボード操作を共通登録で扱うようにした。
  ...
  - `PageUp` / `PageDown` と `Ctrl+Left` / `Ctrl+Right` で前後ファイルへ移動する。
- 全画面化と全画面解除を追加した。
  - `F11` で切り替える。
...
- 2026-06-07 追加方針。
  - ループ再生時に少し目立つ停止期間があるため、デコーダ/音声バッファ/再開処理の待ち時間を減らす方針を検討する。
  - シーク時はバッファ待ちで画面が止まって見えないよう、まずシーク位置の画像を即座に表示し、その後で再生用バッファや音声準備を進める流れを検討する。
  - ループとシークの改善では、映像の即時表示、音声バッファ準備、再生再開の順序を分け、体感上の停止時間を短くすることを優先する。
  - ボスが来たジェスチャーの反応幅は、実操作を見ながら `BOSS_GESTURE_MIN_DISTANCE` と `BOSS_GESTURE_POINT_DISTANCE` を中心に調整する。
- 画像ファイル対応。
