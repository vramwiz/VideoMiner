...
- 2026-06-17 の作業終了時点では、サムネイル一覧モードは最小入口まで実装済み。
  ...
  - クリックで動画を切り替える処理、キーボード上下左右選択、`Enter` 再生、サムネイル画像生成、メモリキャッシュ、遅延生成は未実装。
  - 次回は、まずタイルクリックを既存の動画切り替え処理へ流すところから再開するとよい。
  - Debug Win64 ビルドは `D:\DelphiProg\test\VideoMiner\VideoMiner.dproj` で成功済み。警告 0、エラー 0。
